...
def _help_nav_rows(page: int, total_pages: int, origin: str) -> list:
    """ردیف دکمه‌های ناوبری صفحه‌بندی راهنما را می‌سازد:
    - صفحه اول: فقط «بعدی ⬅️»
    - صفحات میانی: هر دو دکمه «➡️ قبلی» و «بعدی ⬅️»
    - صفحه آخر: فقط «➡️ قبلی»"""
    nav_row = []
    if page > 1:
        nav_row.append(InlineKeyboardButton(text="➡️ قبلی", callback_data=f"help_page_{page - 1}_{origin}"))
    if page < total_pages:
        nav_row.append(InlineKeyboardButton(text="بعدی ⬅️", callback_data=f"help_page_{page + 1}_{origin}"))
    return [nav_row] if nav_row else []


def _help_keyboard(page: int, total_pages: int, origin: str) -> InlineKeyboardMarkup:
    """کیبورد شیشه‌ای پیام راهنما را بر اساس شماره صفحه فعلی می‌سازد. اگر origin برابر 'prof'
    باشد (یعنی راهنما از داخل پروفایل باز شده)، دکمه «🔙 برگشت به پروفایل» نیز اضافه می‌شود؛ در
    حالت 'std' (باز شده از طریق متن/دستور «راهنمایی بانک») این دکمه نمایش داده نمی‌شود."""
    rows = _help_nav_rows(page, total_pages, origin)
    if origin == "prof":
...
async def cb_prof_help(callback: CallbackQuery):
    ...
    بودن کاربر) و در پیوی راهنمای کامل و شخصی‌سازی‌شده نمایش داده می‌شود. راهنما به‌صورت
    صفحه‌بندی‌شده و از صفحه اول باز می‌شود (دقیقاً همان رفتار متن/دستور «راهنمایی بانک»).
    فقط کاربری که ابتدا دستور /profile را ارسال کرده مجاز به استفاده از این دکمه است."""
    owner_id = _get_profile_owner_id(callback.message)
    if owner_id is not None and callback.from_user.id != owner_id:
        await callback.answer()
    ...
    pages = await _build_help_pages(callback.from_user.id, group_only=group_only)
    try:
        await callback.message.edit_text(
            pages[0],
            reply_markup=_help_keyboard(1, len(pages), "prof"),
            parse_mode="HTML",
        )
    except Exception:
        pass
...
@user_router.message(F.text == "راهنمایی بانک")
async def cmd_help_paginated(message: Message):
    """متن/دستور «راهنمایی بانک»: دقیقاً همان عملکرد کلیک روی دکمه «❓ راهنمایی» در پروفایل را
    دارد و صفحه اول راهنمای صفحه‌بندی‌شده را باز می‌کند (بدون محدودیت سطح دسترسی، مانند دکمه
    راهنمای پروفایل)."""
    user_id = message.from_user.id
    group_only = not is_private(message)
    pages = await _build_help_pages(user_id, group_only=group_only)
    await message.reply(
        pages[0],
        reply_markup=_help_keyboard(1, len(pages), "std"),
        parse_mode="HTML",
    )


@user_router.callback_query(F.data.startswith("help_page_"))
async def cb_help_page(callback: CallbackQuery):
    """کال‌بک جابه‌جایی بین صفحات راهنما: متن پیام موجود را (بدون ارسال پیام جدید) ویرایش کرده و
    صفحه درخواستی را نمایش می‌دهد. فقط کاربری که ابتدا راهنما را باز کرده (فرستنده اصلی پیام
    /profile یا متن «راهنمایی بانک») مجاز به استفاده از این دکمه‌هاست."""
    owner_id = _get_profile_owner_id(callback.message)
    if owner_id is not None and callback.from_user.id != owner_id:
        await callback.answer()
        return
    try:
        _, _, page_str, origin = callback.data.split("_", 3)
        page = int(page_str)
    ...
    try:
        await callback.message.edit_text(
            pages[page - 1],
            reply_markup=_help_keyboard(page, total_pages, origin),
            parse_mode="HTML",
        )
    except Exception:
        pass
...
SHOP_GUIDE_TEXT = (
    "<b>دستورات مهم فروشگاه:🛍️</b>\n\n"
    "<b>ثبت محصول جدید 📦✨</b>\n"
    "دستور <code>/add_product</code> ابزار اصلی شما برای ویترین‌سازی است و به شما اجازه می‌دهد محصولات یا خدمات جدید خود را به فروشگاه اضافه کنید. 🛒\n"
    "<b>💡 نکته راهنما:</b> برای داشتن یک ثبت موفق، حتماً پیش از ارسال دستور، تصویر باکیفیت و قیمت دقیق محصول خود را آماده کنید تا فرایند ثبت بدون وقفه‌ای انجام شود.\n"
    "<i>«در بخش ثبت محصول، اطلاعات و مشخصات زیر دریافت و تنظیم می‌شوند:»</i>\n\n"
    "¹• <b>📸 عکس محصول:</b> تصویر کالا که جهت جلب توجه خریداران در ویترین نمایش داده می‌شود.\n"
    "²• <b>📝 عنوان و توضیحات:</b> نام دقیق و جزئیات کامل محصول برای راهنمایی هرچه بهتر مشتریان.\n"
    "<i>(دقت بکنید اول نام رو باید ارسال کنید و سپس توضیحات رو)</i>\n"
    "³• <b>💵 قیمت:</b> تعیین میزان اعتبار یا مبلغ دریافتی به ازای هر واحد از کالا.\n"
    "⁴• <b>📊 نوع موجودی:</b> تعیین وضعیت موجودی به صورت تک‌عددی، تعداد محدود یا نامحدود.\n"
    "<i>(دقت بکنید که کالا های نامحدود تا زمان عدم حذف از فروشگاه قابل فروشن هستن.)</i>\n"
    "⁵• <b>🚚 نیاز به ارسال:</b> مشخص کردن این‌که آیا کالا نیازمند دریافت آدرس و تحویل فیزیکی است یا خیر.\n"
    "⁶• <b>🔑 شناسه یکتا:</b> کد اختصاصی محصول که پس از اتمام ثبت، توسط ربات جهت پیگیری و مدیریت به شما داده می‌شود.\n\n"
    "▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬\n\n"
    "<b>مدیریت انبار 🏬📋</b>\n"
    "دستور <code>/inventory</code> مرکز کنترل کالاهای شماست و امکان نظارت دقیق و سریع بر وضعیت تمامی محصولات ثبت‌شده را فراهم می‌سازد. ⚡\n"
    "<b>🛠️ قابلیت ویژه:</b> شما می‌توانید قیمت و تعداد موجودی محصولات خود را به‌صورت آنی تغییر دهید؛ بنابراین نیازی به حذف کالا و ثبت مجدد آن برای یک تغییر قیمت ساده نخواهید داشت.\n"
    "<i>«در بخش انبار، قابلیت‌های زیر در دسترس شما قرار می‌گیرد:»</i>\n"
    "• <b>📦 لیست کالاها:</b> مشاهده یکجای تمام محصولات موجود به همراه کد اختصاصی آن‌ها.\n"
    "• <b>🔢 ویرایش موجودی:</b> به‌روزرسانی و افزایش یا کاهش تعداد کالاهای آماده‌ی فروش.\n"
    "• <b>🏷️ تغییر قیمت:</b> اصلاح سریع قیمت فروش محصولات در صورت اعمال تخفیف یا به‌روزرسانی ارزش کالا.\n\n"
    "▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬\n\n"
    "<b>آمار و وضعیت فروشگاه 📊💰</b>\n"
    "دستور <code>/my_shop</code> داشبورد جامع مدیریتی شماست و تمام داده‌های مربوط به عملکرد مالی و وضعیت کل فروشگاه را یکجا در اختیارتان می‌گذارد. 📈\n"
    "<i>«در بخش آمار فروشگاه، اطلاعات زیر در دسترس شما قرار می‌گیرد:»</i>\n"
    "• <b>🏪 مشخصات فروشگاه:</b> مشاهده نام و وضعیت فعال یا غیرفعال بودن کسب‌وکار شما در ربات.\n"
    "• <b>📦 تعداد کل محصولات:</b> مجموع کالاهایی که تا کنون در سیستم تعریف و ثبت کرده‌اید.\n"
    "• <b>🛒 آمار سفارشات:</b> تعداد کل خریدها و معاملات موفقی که از طریق فروشگاه شما انجام شده است.\n"
    "• <b>💳 میزان کل درآمد:</b> مجموع اعتبارات و مبالغ کسب‌شده از فروش محصولات که به‌طور دقیق محاسبه شده است.\n\n"
    "▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬\n\n"
    "<b>حذف محصول 🗑️❌</b>\n"
    "دستور <code>/delete</code> راهکاری سریع برای از دسترس خارج کردن کالاهایی است که دیگر قصد عرضه‌ی آن‌ها را در فروشگاه ندارید. ⚠️\n"
    "<b>🔒 حفظ سوابق:</b> با حذف یک محصول، تنها ویترین مربوط به آن پاک می‌شود و تاریخچه خریدهای قبلی کاربران جهت حفظ حسابداری و گزارش‌های مالی، در دیتابیس محفوظ باقی می‌ماند.\n"
    "<i>«مشخصات و نحوه اجرای این دستور:»</i>\n"
    "• <b>🔢 فرمت ارسال:</b> این دستور باید حتماً همراه با کد اختصاصی محصول ارسال شود (به عنوان مثال: /delete 105).\n"
    "• <b>🚫 توقف فروش:</b> پس از اجرای دستور، کالا بلافاصله از فهرست فروشگاه حذف شده و امکان ثبت سفارش جدید برای آن وجود نخواهد داشت.\n"
)


@user_router.message(F.text == "راهنمای فروشگاه")
async def cmd_shop_guide(message: Message):
    """متن «راهنمای فروشگاه»: فقط در پیوی و فقط برای کاربرانی که فروشگاه (تأییدشده) دارند یا
    سوپرادمین هستند پاسخ داده می‌شود؛ در غیر این صورت (گروه، کاربر عادی، ادمین بدون فروشگاه،
    پستچی بدون فروشگاه) ربات هیچ واکنش یا پیامی ارسال نمی‌کند."""
    if not is_private(message):
        return
    user_id = message.from_user.id
    if not is_super_admin(user_id):
        async with aiosqlite.connect(DB_PATH) as db:
            async with db.execute(
                ...
                has_shop = (await cur.fetchone()) is not None
        if not has_shop:
            return
    await message.reply(SHOP_GUIDE_TEXT, parse_mode="HTML")


COURIER_GUIDE_TEXT = (
    "<b>دستورات مهم پستچی</b>\n\n"
    "<b>مشاهده سفارش‌های آماده ارسال 🚚📦</b>\n"
    ...
    "▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬\n\n"
    "<b>ثبت تحویل نهایی سفارش ✅🔢</b>\n"
    "دستور <code>/confirm_dispatch [کد]</code> نهایی‌کننده کامل چرخه ارسال است و تحویل موفقیت‌آمیز کالا به خریدار را با استفاده از کد ۱۰ رقمی تایید در سیستم ثبت می‌کند. ⚡\n"
    "<b>🔒 نکته امنیتی:</b> جهت جلوگیری از هرگونه خطا، تنها زمانی اقدام به ارسال این دستور کنید که کالا را تحویل داده و کد ۱۰ رقمی اختصاصی را از خریدار دریافت کرده باشید.\n"
    "<i>«مشخصات و نحوه اجرای این دستور:»</i>\n"
    "• <b>🔑 تایید با کد ۱۰ رقمی:</b> وارد کردن کد اختصاصی ۱۰ رقمی خریدار همراه با دستور جهت ثبت قطعی تحویل (به عنوان مثال: /confirm_dispatch 1234567890).\n"
    "• <b>🎯 مختومه شدن سفارش:</b> تغییر وضعیت نهایی مرسوله به «تحویل داده شد» در دیتابیس ربات.\n"
    "• <b>💳 ثبت عملکرد:</b> ذخیره سابقه تحویل موفق در حساب پستچی و آزادسازی اعتبار مربوط به ارسال.\n"
)


@user_router.message(F.text == "راهنمای پستچی")
async def cmd_courier_guide(message: Message):
    """متن «راهنمای پستچی»: فقط در پیوی و فقط برای کاربرانی که پستچی هستند یا سوپرادمین‌اند
    پاسخ داده می‌شود؛ در غیر این صورت (گروه، کاربر عادی، ادمین بدون سمت پستچی، فروشنده بدون
    سمت پستچی) ربات هیچ واکنش یا پیامی ارسال نمی‌کند."""
    if not is_private(message):
        return
    user_id = message.from_user.id
    if not is_super_admin(user_id):
        async with aiosqlite.connect(DB_PATH) as db:
            async with db.execute(
                "SELECT 1 FROM couriers WHERE user_id = ?", (user_id,)
            ) as cur:
                is_courier = (await cur.fetchone()) is not None
        if not is_courier:
            return
...
async def _build_help_pages(user_id: int, group_only: bool = False) -> list:
    """نسخه صفحه‌بندی‌شده راهنما را می‌سازد و لیستی از متن هر صفحه را برمی‌گرداند. محتوای هر
    بخش دقیقاً همان محتوای _build_help_text است؛ فقط برای نمایش در قالب صفحه‌بندی‌شده (با دکمه‌های
    «➡️ قبلی» / «بعدی ⬅️») به چند صفحه مجزا تقسیم شده است. تعداد صفحات بر اساس سطح دسترسی واقعی
    کاربر (فروشگاه تأییدشده، پستچی، ادمین، سوپرادمین) به‌صورت پویا تعیین می‌شود."""
    profile_page = (
        "<b>پروفایل👤💳</b>\n"
        "دستور پروفایل کلیدی‌ترین بخش ربات است و به عنوان دروازه اصلی دسترسی شما به تمامی امکانات بانک عمل می‌کند. ⚡\n\n"
        "<b>🔒 نکته امنیتی:</b> جهت حفظ حریم شخصی و امنیت اطلاعات مالی شما، برخی از جزئیات این دستور تنها در پیوی ربات به شما نمایش داده می‌شوند.\n\n"
        "<i>«در بخش پروفایل، اطلاعات زیر در دسترس شما قرار می‌گیرد:»</i>\n\n"
        "•      <b>🏷️ نام:</b> نام کاربری اختصاصی شما که در سیستم ثبت شده است.\n\n"
        "•      <b>🔢 شماره حساب:</b> مهم‌ترین شناسه بانکی شما؛ برای انجام تمامی امور مالی مانند انتقال وجه، دریافت وام و خدمات مشابه، به این شماره نیاز خواهید داشت.\n\n"
        "•      <b>💰 موجودی:</b> میزان دارایی فعلی شما. در کسب و افزایش سرمایه هیچ محدودیتی ندارید و تمام اعتبار شما به طور دقیق در این بخش ثبت می‌شود.\n\n"
        "•      <b>⚙️ وضعیت حساب:</b> نشان‌دهنده فعال یا غیرفعال بودن حساب شماست. تا زمانی که حساب شما فعال باشد، امکان تبادلات مالی را خواهید داشت؛ اما در صورت تخطی از قوانین مدرسه، ممکن است حساب شما «فریز» (مسدود) شده و موقتاً از دسترسی به خدمات ربات محروم شوید. ⚠️\n\n"
    )

    bank_loan_page = (
        "<b>بانک آترامنتوم 🏛️💰</b>\n"
        "بخش بانک آترامنتوم امکان سپرده‌گذاری با سود روزانه مشخص را برای شما فراهم می‌سازد. در این سیستم می‌توانید تا سقف مجاز (که ممکن است به مرور زمان تغییر کند) دارایی خود را سپرده‌گذاری کرده و هر شب سود آن را دریافت نمایید.\n\n"
        "<b>⚠️ توجه ویژه:</b> چنانچه مجموع سود دریافتی شما از سقف تعیین‌شده فراتر رود، مبلغ مازاد مستقیماً به حساب خزانه واریز شده و غیرقابل استرداد خواهد بود.\n\n"
        "<b>وام‌های آترامنتوم 💳💎</b>\n"
        "در صورت نیاز به تامین سرمایه برای خرید کالا، شرکت در مسابقات یا سایر امور، می‌توانید از تسهیلات وام آترامنتوم استفاده کنید.\n\n"
        "•      <b>🚫 محدودیت دریافت:</b> در یک زمان واحد، امکان دریافت دو وام وجود ندارد و تنها می‌توانید یک وام فعال داشته باشید.\n\n"
        "•      <b>⚙️ فرآیند درخواست:</b> پس از فشردن دکمه درخواست وام، مبلغ مورد نظر را در بازه عددی مشخص‌شده انتخاب می‌کنید. میزان سود تسهیلات بر اساس مبلغ انتخابی شما محاسبه شده و در ادامه، تعداد اقساط و نوع وام را تعیین می‌نمایید.\n\n"
        "<b>انواع تسهیلات وام:</b>\n"
        "•      <b>🔒 وام وثیقه‌ای:</b> در این روش، بخشی از دارایی شما تا زمان پرداخت آخرین قسط در بانک مسدود (بلوکه) می‌ماند. همچنین جهت ارزیابی تمکن مالی، لازم است درصد مشخصی از مبلغ وام درخواستی در حساب شما موجود باشد.\n\n"
    ...
    assets_tracking_page = (
        "<b>لیست دارایی‌های من 📦💎</b>\n"
        "در این بخش می‌توانید تمامی اموال و دارایی‌های ثبت‌شده خود را همراه با تصویر و توضیحات کامل مشاهده نمایید.\n\n"
        "•      <b>📊 ترتیب نمایش:</b> چیدمان دارایی‌ها بر اساس ارزش مالی، از گران‌ترین به ارزان‌ترین تنظیم شده است.\n\n"
        "•      <b>🔢 تعداد کالا:</b> در صورت مالکیت چند نمونه از یک محصول، تعداد دقیق آن به شما نشان داده می‌شود.\n\n"
        "•      <b>🛍️ مدل فروش محصولات:</b> اقلام فروشگاه به سه دسته عمده، محدود و تکی تقسیم می‌شوند. توجه داشته باشید که امکان خرید چندتایی تنها برای محصولات «عمده» فراهم است و مابقی موارد دارای محدودیت خرید هستند.\n\n"
        "<b>پیگیری مرسوله 🚚📍</b>\n"
        "پس از ثبت خرید (در صورتی که کالای انتخابی نیاز به ارسال پستی داشته باشد)، یک کد پیگیری اختصاصی برای شما صادر می‌شود.\n\n"
        "<b>📌 راهنمای استفاده:</b> جهت اطلاع از آخرین وضعیت و مرحله تحویل سفارش، کافی است ابتدا روی دکمه پیگیری مرسوله کلیک کرده و سپس کد پیگیری دریافتی را ریپلای (Reply) نمایید.\n\n"
    )

    shop_intro_page = (
        "<b>فروشگاه و کسب‌وکار 🛒🏪</b>\n"
    ...
    if group_only:
        # صفحه ۱: معرفی پروفایل - صفحه ۲: بانک و وام آترامنتوم - صفحه ۳: دارایی‌ها و پیگیری مرسوله -
        # صفحه ۴: فروشگاه و کسب‌وکار (بدون درنظرگرفتن سطح دسترسی یا ادمین بودن کاربر)
        return [profile_page, bank_loan_page, assets_tracking_page, shop_intro_page]
    ...
    pages = [profile_page, bank_loan_page, assets_tracking_page, shop_intro_page]
...
async def cmd_help(message: Message):
    """دستور /help (و متن مترادف «راهنمای جامع بانک») فقط برای ادمین‌ها و سوپرادمین‌ها پاسخ
    می‌دهد؛ اگر کاربران عادی، افراد دارای فروشگاه یا پستچی‌ها این دستور را ارسال کنند، ربات هیچ
    واکنش یا پیامی نمایش نمی‌دهد."""
    if not await check_admin_filter(message):
        return
